...
class LeoLayer2(lasagne.layers.Layer):
    def __init__(self, incoming, num_units, W=lasagne.init.Normal(0.01), **kwargs):
        ...
        print("leo2", self.input_shape, num_units)
...
class LeoLayer3(lasagne.layers.Layer):
    def __init__(self, incoming, num_units, W=lasagne.init.Normal(0.01), **kwargs):
        ...
        self.I = np.product(self.input_shape[1:])
        self.H = num_units
        self.desired_output = num_units
        tmp = incoming #T.reshape(incoming, [self.input_shape[0], self.I])
        if self.I <= self.H:
            self.k = self.I
            #LeoLayer2(network, num_units=256, name='my_dot_layer')
            self.c = [LeoLayer2(tmp, num_units=self.k, name = name + '_tmp_%d' % i) if (i + 1) * self.k <= self.H \
                      else LeoLayer2(tmp, num_units=self.H - i * self.k, name = name + '_tmp_%d' % i) \
                      for i in xrange((self.H + self.I - 1) / self.I)]
        else:
            self.k = self.H
            self.c = [LeoLayer2(tmp[:,i*self.k:(i+1)*self.k], num_units=self.k, name = name + '_tmp_%d' % i) if (i + 1) * self.k <= self.I \
                      else LeoLayer2(tmp[:,i*self.k:self.I], num_units=self.k, name =  name + '_tmp_%d' % i) \
                      for i in xrange((self.I + self.H - 1) / self.H)]

    def get_output_for(self, input, **kwargs):
        a = T.zeros((input.shape[0], self.H), dtype=input.dtype)
        if self.I <= self.H:
            for i in xrange((self.H + self.I - 1) / self.I): # ceil(5/3)=2, i={0,1} -- {0,3}
                end = (i + 1) * self.k
                if end > self.H:
                    end = self.H
                a[:, i*self.k:end] = self.c[i]
        else:
            for i in xrange((self.I + self.H - 1) / self.H):
                a += self.c[i]
        return a
...
class LeoLayer3(lasagne.layers.Layer):
    def __init__(self, incoming, num_units, W=lasagne.init.Normal(0.01), **kwargs):
        ...
        if num_units < self.num_inputs:
            self.k = int(self.num_inputs / num_units)
            self.n = num_units
        else:
            self.k = int(num_units / self.num_inputs)
            self.n = self.num_inputs
        self.num_units = self.k * self.n
        self.W = self.add_param(W, (self.k, self.n), name='LeoLayer3_W')
        print("leo_love_you!", self.k, self.n)


    def get_output_for(self, input, **kwargs):
        fft_w = fft.rfft(T.reshape(self.W, [self.k, self.n]))
        if self.desired_output < self.num_inputs:
            r = T.zeros([input.shape[0], self.n], dtype=input.dtype)
            x = T.zeros([input.shape[0], self.num_units], dtype=input.dtype)
            x = T.set_subtensor(x[:,:self.num_inputs], T.reshape(input, [input.shape[0], self.num_inputs]))
            fft_x = fft.rfft(T.reshape(x, [input.shape[0], self.k, self.n]))
            for i in xrange(self.k):
                r = T.inc_subtensor(r[:,:], self.aux(fft_w[i,:,:], fft_x[:,i,:,:]))
            r = T.reshape(r, [input.shape[0], self.n])
        else:
            r = T.zeros((input.shape[0], self.num_units), dtype=input.dtype)
            x = T.reshape(input, [input.shape[0], self.num_inputs])
            fft_x = fft.rfft(x) 
            for i in xrange(self.k):
                r = T.set_subtensor(r[:,i*self.n:(i+1)*self.n], self.aux(fft_w[i,:,:], fft_x))
            r = T.reshape(r, [input.shape[0], self.num_units])

        return r[:,:self.desired_output]


    def aux(self, fft_w, fft_x):
        x = T.reshape(fft_w[..., 0], [self.n/2+1])
        y = T.reshape(fft_w[..., 1], [self.n/2+1])

        u = T.reshape(fft_x[..., 0], [fft_x.shape[0], self.n/2+1])
        v = T.reshape(fft_x[..., 1], [fft_x.shape[0], self.n/2+1])

        r = T.stack([u * x - v * y, v * x + u * y], axis=2)
        return T.reshape(fft.irfft(r), [fft_x.shape[0], self.n])
...
class LeoLayer3(lasagne.layers.Layer):
    ...
    def get_output_for(self, input, **kwargs):
        ...
        tmp_x = T.set_subtensor(tmp_x[:,:self.num_inputs], input.reshape([input.shape[0], self.num_inputs]))
        if self.desired_output > self.num_inputs:
        ...
        #if self.desired_output <= self.num_inputs:
        #    # [w1, ..., wk] x
        #    r = T.sum(self.W[self.indx] * x, axis=0)
        #else:
        #    r = T.sum(self.W[self.indx] * x.reshape(input.shape[0], self.k, self.n).transpose([1,0,2]), axis=0)

        fft_w = fft.rfft(T.reshape(self.W, [self.k, 1, self.n]))
        x = fft_w[..., 0]
        y = fft_w[..., 1]
        x = T.reshape(x, [self.k, 1, self.num_units/2 + 1])
        y = T.reshape(y, [self.k, 1, self.num_units/2 + 1])
...
def build_cnn(input_var=None):
    ...
    # network = LeoLayer(network, num_units=256, name='my_dot_layer')
    network = LeoLayer3(network, num_units=1024, name='my_dot_layer')
