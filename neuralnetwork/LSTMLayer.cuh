...
public:
    ...
    int sigmoid_driv(Mat* x, Mat* y) { // return x * (1 - x)
        assert(0 == add_mult(y, x, -1)); // y = - x
        assert(0 == add_scalar(y, 1, y)); // y = 1 - x
        return mult_elementwise(y, x, y, 0); // y = (1-x)*x
    }
