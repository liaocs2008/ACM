...
class CircFC(object):
    ...
    def forward(self, x):
        a = np.fft.ifft( np.fft.fft(x) * np.fft.fft(self.r) ) # a = dot(x, R.T)
        if __debug__:
            check_a = np.dot(x, circulant(self.r).T)
    ...
    def backward(self, x, d_a):
        ...
        # not like following
        #d_x = np.fft.ifft(np.fft.fft(d_a)*np.fft.fft(self.r))
        d_x = np.fft.ifft( np.fft.fft(d_a) * np.fft.fft(self.r[self.mapping]) )
        if __debug__:
            check_d_x = np.dot(d_a, circulant(self.r))
...
def GradientChecking3():
    ...
    checklist = [layers[0].r]
    grads_analytic = [layers[0].dr]
    names = ['r']
    for j in xrange(len(checklist)):
        mat = checklist[j]
        dmat = grads_analytic[j]
