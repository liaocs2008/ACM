...
def search(strings, token, strings_pos = None):
  ...
  if not strings_pos:
    strings_pos = []
    leftmost = bisect.bisect_left(strings, [token]) - 1
    if leftmost < 0 : leftmost = 0
    #print "NONE STRING POS, left=", leftmost-1, "len=", len(strings)
    for i in xrange(leftmost, len(strings)):
      check = False
      #print "i=", i, "string=",strings[i]
      for s in strings[i]:
    ...
    tmp_strings_pos = []
    tmp_strings = map(lambda i: strings[i], strings_pos)
    leftmost = bisect.bisect_left(tmp_strings, [token]) - 1
    if leftmost < 0 : leftmost = 0
    for i in xrange(leftmost, len(strings_pos)):
      check = False
...
if __name__ == "__main__":
  ...
  #while True:
    ...
    if command[0] == "ADD":
      (type, id, score, string) = (command[1], command[2], float(command[3]), sorted(map(lambda x:x.lower(), command[4:]), reverse=True))
      database[id] = {'score': score, 'type':type, 'time':datetime.datetime.now()}
      tmp = zip(strings, ids)
      bisect.insort(tmp, (string, id))
      strings, ids = zip(*tmp)
